local function set_alacritty_config(config, ...) vim.system { 'alacritty', 'msg', 'config', config, ... } end
...
local terminals = {
  alacritty = {
    cond = function() return vim.fn.getenv 'TERM' == 'alacritty' end,
    enter = function(bg_color)
      set_alacritty_config('window.padding = { x = 0, y = 0 }', 'colors.primary.background = "' .. bg_color .. '"')
    end,
    exit = function() set_alacritty_config '-r' end,
  },
  wezterm = {
    cond = function() return vim.fn.getenv 'TERM_PROGRAM' == 'WezTerm' end,
    enter = function(bg_color) set_wezterm_var('vim-enter', bg_color) end,
    exit = function() set_wezterm_var 'vim-exit' end,
  },
}
...
return {
  ...
  setup = function(terms)
    ...
    local _, term = vim.iter(pairs(terminals)):find(function(k, v) return vim.list_contains(terms, k) and v.cond() end)
    ...
    vim.api.nvim_create_autocmd('VimLeavePre', {
      callback = function() term.exit() end,
      group = group,
    })
