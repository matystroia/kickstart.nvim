...
local function tick(buf)
  ...
  local node_types = {
    {
      ...
      render = function(n) return vim.treesitter.get_node_text(n:field('name')[1], buf) end,
      hl_group = 'Type',
    },
    {
      types = { 'impl_item' },
      render = function(n) return vim.treesitter.get_node_text(n:field('type')[1], buf) end,
      hl_group = 'Type',
    },
    {
    ...
    },
    {
      ...
      render = function(n) return vim.treesitter.get_node_text(n:field('type_name')[1], buf) end,
      hl_group = 'Type',
    },
  }
  ...
  local ret = vim
    ...
    :map(function(n)
      local type = n:type()
      local node_type = vim.iter(node_types):find(function(t) return vim.list_contains(t.types, type) end)

      if node_type == nil then return nil end
...
M.setup = function()
  ...
  vim.api.nvim_create_autocmd('BufEnter', {
    ...
    callback = function()
      ...
      if vim.b._contextline_active == nil then vim.b._contextline_active = has_parser(buf) end

      if vim.b._contextline_active == false then return end

      local wrapped_tick = vim.schedule_wrap(tick)
      timer:start(0, 500, function() wrapped_tick(buf) end)
    end,
  })
...
M.context = function() return vim.b._contextline_active and state.context or '' end
